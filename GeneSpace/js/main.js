...
function createAtmos(colors, vertex_text, fragment_text) {
    ...
    for (var j = 0; j < colors.length; j++) {
        ...
        var normalColors = new THREE.Color(R, G, B);
        colorsRGBLight.push(normalColors);
    }

    atmouniforms.fresnelExp.value = randomRange(0.1, 5);
    atmouniforms.atmoThickness.value = randomRange(1.0, 1.5);
    ...
    atmo = new THREE.Mesh(new THREE.IcosahedronGeometry(
        planetSize * 1.04, 4), atmoMaterial);
...
function createPlanet(start, vertex_text, fragment_text) {
    ...
    planetTextInfo = generateName(planet, "30px", -1000, true, planetData.colors, planetData.regionsInfo);
    planetTextInfo.setWidthbyPercent(75);
    planetTextInfo.setHeight(planetSize);

    targetPoint.object = planet;
    targetPoint.size = planetSize * 0.75;
}
...
function generateName(parent, fontsize, left, isInfo, colorpallette, regions) {
    ...
    var roll = randomRange(0, 10);
    var newText = createTextLabel(fontsize, left, label);
